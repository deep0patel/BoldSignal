...
# ── Atlas / ROI (main process only — DataLoader subprocesses skip this) ──────
...
def get_model(config):
    ...
    if 'config_update' in sig:
        # Feature extractors don't support 'mps' — fall back to cpu for extraction.
        # Brain model inference still runs on DEVICE (mps/cuda).
        # Do NOT swap model_names: the checkpoint projectors are fixed to the
        # dimensions of the original training encoders (vjepa2-vitg, Llama-3.2-3B).
        _extractor_device = 'cpu' if DEVICE == 'mps' else DEVICE
        kw['config_update'] = {
            'data.audio_feature.device':       _extractor_device,
            'data.text_feature.device':        _extractor_device,
            'data.video_feature.image.device': _extractor_device,
        }
